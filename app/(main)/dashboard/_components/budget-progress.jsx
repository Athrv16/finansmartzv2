    ...
    <Card className="border-border/60 bg-white/85 shadow-sm dark:bg-slate-900/70">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 border-b border-border/60 pb-3">
        <div className="flex-1">
          <CardTitle className="text-base font-semibold">Monthly Budget (Default Account)</CardTitle>
      ...
      <CardContent className="pt-4">
        {initialBudget && (
          <div className="space-y-2">
            ...
            />
            <p className="text-right text-xs text-muted-foreground">{percentUsed.toFixed(1)}% used</p>
          </div>
        )}
        {!initialBudget && (
          <div className="rounded-lg border border-dashed p-4 text-xs text-muted-foreground">
            Set a monthly budget to see usage progress and alerts.
          </div>
        )}
      </CardContent>
    </Card>
