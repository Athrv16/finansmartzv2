...
export const sendBillReminders = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const recurringBills = await step.run("fetch-recurring-bills", async () => {
      return await db.bill.findMany({
        where: {
          isRecurring: true,
        },
    ...
    for (const bill of bills) {
      if (bill.lastReminderSent && bill.lastReminderSent >= todayStart) continue;
      addReminder(bill.user, {
        name: bill.name,
        amount: `₹${Number(bill.amount).toFixed(0)}`,
        dueDate: bill.nextDueDate.toLocaleDateString("en-IN"),
      });
      if (remindersByUser.has(bill.userId)) {
        remindersByUser.get(bill.userId).billIds.add(bill.id);
      }
    }

    for (const bill of recurringBills) {
      let nextDue = bill.nextDueDate;
      if (nextDue < todayStart && bill.recurringInterval) {
        let candidate = new Date(nextDue);
        while (candidate < todayStart) {
          candidate = calculateNextRecurringDate(candidate, bill.recurringInterval);
        }
        nextDue = candidate;
        await db.bill.update({
          where: { id: bill.id },
          data: { nextDueDate: candidate },
        });
      }

      if (nextDue >= todayStart && nextDue <= windowEnd) {
        if (bill.lastReminderSent && bill.lastReminderSent >= todayStart) continue;
        addReminder(bill.user, {
          name: bill.name,
          amount: `₹${Number(bill.amount).toFixed(0)}`,
        ...
        if (remindersByUser.has(bill.userId)) {
          remindersByUser.get(bill.userId).recurringBillIds.add(bill.id);
        }
      }
    }
    ...
    for (const entry of remindersByUser.values()) {
      ...
      const billIds = Array.from(entry.billIds);
      if (billIds.length > 0) {
      ...
      const recurringBillIds = Array.from(entry.recurringBillIds);
      if (recurringBillIds.length > 0) {
        await db.bill.updateMany({
          where: { id: { in: recurringBillIds } },
          data: { lastReminderSent: new Date() },
        });
      }
