...
const EDUCATIONAL_SOURCES = [
  {
    ...
    url: "https://www.investor.gov/glossary-term-categories/asset-allocation",
  },
  {
    label: "SEBI Investor: Diversification through fund structures",
    url: "https://investor.sebi.gov.in/fund_of_fund.html",
  },
  {
    label: "Vanguard: Asset allocation models",
...
const buildStrategySummary = (risk, horizonYears, profile, allocation) => {
  ...
  if (savingsRate !== null && savingsRate <= 0) {
    return {
      title: "Stabilize cash flow before taking more market risk",
      description:
        "Because expenses are currently absorbing all income or more, the priority should be rebuilding monthly surplus, improving liquidity, and keeping any investment mix defensive.",
    };
  }

  if (emergencyFundMonths !== null && emergencyFundMonths < 3) {
    return {
      title: "Build a cash buffer before leaning into growth assets",
      description:
        "Your profile suggests a thinner emergency cushion, so the strategy should stay measured until 3 to 6 months of essential expenses are protected.",
    };
  }

  if (risk === "conservative") {
    return {
      title: "Capital preservation with diversified exposure",
      description:
        `This profile leans toward stability first, using a higher bond and cash mix with selective equity exposure. A shorter or moderate time horizon makes downside control more important than maximizing return.`,
    };
  }

  if (risk === "aggressive") {
    return {
      title: "Long-horizon growth with volatility tolerance",
      description:
        `This strategy leans heavily toward equities because the selected risk profile and time horizon support accepting market swings in pursuit of higher long-term growth potential.`,
    };
  }

  return {
    title: "Balanced growth with risk spreading",
    description:
      `This approach uses a diversified mix of equities, bonds, and smaller stabilizers like cash or alternatives. With a ${horizon}-year horizon, the aim is to balance compounding potential with smoother drawdowns.`,
  };
};

const buildStrategyHighlights = (profile, allocation) => {
  const highlights = [];

  if ((allocation?.stocks ?? 0) >= 65) {
    highlights.push("Equity-heavy allocation for stronger long-term growth potential.");
  } else if ((allocation?.stocks ?? 0) >= 45) {
    highlights.push("Balanced equity exposure to pursue growth without going fully aggressive.");
  } else {
    highlights.push("Lower equity allocation to reduce volatility and protect capital.");
  }

  if ((allocation?.bonds ?? 0) >= 30) {
    highlights.push("Meaningful bond exposure to support stability and smoother portfolio behavior.");
  }

  if ((allocation?.cash ?? 0) >= 8) {
    highlights.push("Extra cash allocation to improve flexibility for near-term needs or uncertainty.");
  }

  if ((allocation?.alternatives ?? 0) > 0) {
    highlights.push("A small alternatives sleeve for diversification beyond stocks and bonds.");
  }

  if (profile?.savingsRate !== null && profile?.savingsRate >= 0.2) {
    highlights.push("Healthy savings rate supports disciplined periodic investing.");
  }
...
const buildInvestmentRecommendations = async ({ profile, riskTolerance, horizonYears }) => {
  ...
  return {
    ...
    allocation,
    strategy: buildStrategySummary(risk, horizon, profile, allocation),
    strategyHighlights: buildStrategyHighlights(profile, allocation),
    recommendations: attachQuotes(recommendations, quotes),
    warnings,
    notes,
    sources: EDUCATIONAL_SOURCES,
